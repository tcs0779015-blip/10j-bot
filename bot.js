...
Keep it short. Format using Markdown.
...
async function handleAudioUpload(message, attachment) {
  ...
  if (!period.notes) {
    return channel.send(`ℹ️ **${period.name}** doesn't need notes.`);
  }
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name: '📄 Files', value: '• `Notes PDF` — full formatted notes\n• `Transcript TXT` — raw transcript', inline: false })
      .setFooter({ text: 'Class 10J • New Indian Model School' })
      .setTimestamp();
    ...
    await processingMsg.edit(`❌ Error: ${err.response?.data?.error?.message || err.message}`);
  }
}
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (audioAttachment) return handleAudioUpload(message, audioAttachment);
  ...
  if (content.toLowerCase().startsWith('/teacher')) {
    ...
    await updatePeriodCard(active.key);
    return message.reply(`✅ Logged **${name}** for **${active.period.name}**.`);
  }
  ...
  if (content.toLowerCase() === '/help') {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🎵 Upload audio file',   value: 'Drag & drop any .mp3 .m4a .wav — bot auto-detects period and processes it', inline: false },
        { name: '/teacher [name]',         value: 'Log who joined (e.g. `/teacher Mr. Khan`)', inline: false },
        { name: '/proxy',                  value: 'Mark current period as proxy/empty', inline: false },
